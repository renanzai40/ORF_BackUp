...
- **(src/orf/commands/apply_xliff.py)**: the final converter `validate_input` gate now honors `--force` (`if not force and not converter.validate_input(...)`). Previously the two CLI-level skeleton checks (extension + ZIP content-level) warned-and-proceeded under `--force`, but this last gate re-rejected raw cross-format skeletons (e.g. a raw `.docx` with `--format pptx --force`) with "Input file ... is not valid for pptx format" — exit 1 despite the force contract being "warn + proceed". Locked in by `tests/test_apply_xliff_format_validation.py::TestForceBypassesFinalConverterGate` (RED→GREEN); non-force rejection and both force warnings are unchanged. (Resolves e2e-test-suite#86)
- **(src/orf/channels/xliff2docx/parser.py)**: E2E-06b double-escape leak closed at the parse boundary. When the serialized XLIFF target carries inline markup escaped twice (`&amp;lt;bx .../&amp;gt;`), lxml's `itertext()` resolves only the first layer, leaving the literal string `&lt;bx .../&gt;` — which none of `_strip_inline_tags`, `build_formatted_runs`, or the `<bx|<ex` guards recognize (they require a real `<`), so the entity text reached `<w:t>` verbatim and the user saw raw markup in the DOCX. New `_target_text()` extraction helper applies one `xml.sax.saxutils.unescape` (incl. `&quot;`) after `_strip_wrapper` at all three target extraction points (1.1/1.2 trans-unit, 2.0 segment, 2.0 unit), so every downstream backfill path (position / non_body / text-match / fuzzy / inline) sees real `<bx>`/`<ex>` markup and bold/italic formatting is applied instead of leaked. Single-escaped targets (`&lt;bx`) are byte-identical after the fix (unescape is a no-op on already-resolved text). Locked in by `tests/test_xliff2docx_bx_ex_leak.py::TestDoubleEscapedBxEx` (RED on HEAD parser: 1 failed/13 passed with entities visible; GREEN after: 14 passed); E2E-07 fuzzy matching and E2E-70 whitespace behavior unchanged. Supersedes the stale `e2e-06b-fix` branch (its `_backfill_translation`-only unescape missed the position-based path that carried the original bug, and targets the pre-refactor monolith).
